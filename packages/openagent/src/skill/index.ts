...
import { SkillPlugin } from "@openagent-ai/core/plugin/skill"
import { Permission } from "@/permission"
import { FSUtil } from "@openagent-ai/core/fs-util"
import { Config } from "@/config/config"
import { FrontmatterError } from "@openagent-ai/core/v1/config/error"
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, OPENAGENT_SKILL_PATTERN)
  }
...
export class Service extends Context.Service<Service, Interface>()("@openagent/Skill") {}
...
export function fmt(list: Info[], opts: { verbose: boolean }) {
  ...
  if (described.length === 0) return "当前没有可用 skills。"
  if (opts.verbose) {
    return [
      "<available_skills>",
      ...described
