...
import { OpenAgentHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for openagent serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(OpenAgentHttpApi)
...
export const PublicApi = OpenAgentHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "openagent",
    version: "1.0.0",
    description: "openagent api",
