...
import {
  ...
  type JsonSchema,
  type LLMEvent,
} from "@openagent-ai/llm"
import type { LLMClientShape } from "@openagent-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "deepseek") return { type: "unsupported", reason: "provider is not deepseek" }
  const npm = input.model.api.npm
  if (npm !== "@ai-sdk/openai-compatible")
    return { type: "unsupported", reason: "provider package is not OpenAI-compatible" }
  if (input.auth?.type === "oauth") return { type: "unsupported", reason: "DeepSeek only supports API key auth" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @openagent-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains openagent-owned. The native runtime only adapts
      // the @openagent-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
