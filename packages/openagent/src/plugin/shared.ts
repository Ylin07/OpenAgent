...
export const DEPRECATED_PLUGIN_PACKAGES = ["openagent-openai-codex-auth", "openagent-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, openagentVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(openagentVersion) || semver.major(openagentVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.openagent
  if (typeof range !== "string") return
  if (!semver.satisfies(openagentVersion, range)) {
    throw new Error(`Plugin requires openagent ${range} but running ${openagentVersion}`)
  }
}
