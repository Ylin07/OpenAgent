...
import type { ConsoleState } from "@openagent-ai/core/v1/config/console-state"
import { FSUtil } from "@openagent-ai/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest, HttpClientResponse } from "effect/unstable/http"
import { EffectFlock } from "@openagent-ai/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@openagent-ai/core/v1/config/config"
import { ConfigPermissionV1 } from "@openagent-ai/core/v1/config/permission"
import { ConfigPluginV1 } from "@openagent-ai/core/v1/config/plugin"
...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in openagent config are deprecated; move them to tui.json", { path: source })
  return copy
}
...
export class Service extends Context.Service<Service, Interface>()("@openagent/Config") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://openagent.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.OPENAGENT_CONFIG && !Flag.OPENAGENT_CONFIG_DIR && !Flag.OPENAGENT_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://openagent.ai/config.json" }, null, 2))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://openagent.ai/config.json"
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/openagent`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://openagent.ai/config.json"
        ...
        if (Flag.OPENAGENT_CONFIG) {
          yield* merge(Flag.OPENAGENT_CONFIG, yield* loadFile(Flag.OPENAGENT_CONFIG, authEnv))
          log.debug("loaded custom config", { path: Flag.OPENAGENT_CONFIG })
        }
        ...
        if (Flag.OPENAGENT_CONFIG_DIR) {
          log.debug("loading config from OPENAGENT_CONFIG_DIR", { path: Flag.OPENAGENT_CONFIG_DIR })
        }
        ...
        if (process.env.OPENAGENT_CONFIG_CONTENT) {
          ...
          log.debug("loaded custom config from OPENAGENT_CONFIG_CONTENT")
        }
        ...
        if (Flag.OPENAGENT_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.OPENAGENT_PERMISSION))
          } catch (err) {
            log.warn("OPENAGENT_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.OPENAGENT_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.OPENAGENT_DISABLE_PRUNE) {
