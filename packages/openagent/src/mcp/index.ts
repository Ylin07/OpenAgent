...
import { ConfigMCPV1 } from "@openagent-ai/core/v1/config/mcp"
...
export class Service extends Context.Service<Service, Interface>()("@openagent/MCP") {}
    ...
    const connectTransport = (transport: Transport, timeout: number) =>
      Effect.acquireUseRelease(
        ...
        (t) =>
          Effect.tryPromise({
            try: () => {
              const client = new Client({ name: "openagent", version: InstallationVersion })
            ...
            if (isAuthError) {
              ...
              if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
                ...
                return events
                  .publish(TuiEvent.ToastShow, {
                    title: "MCP Authentication Required",
                    message: `Server "${key}" requires authentication. Run: openagent mcp auth ${key}`,
                    variant: "warning",
                    duration: 8000,
                  })
                  .pipe(Effect.ignore, Effect.as(undefined))
