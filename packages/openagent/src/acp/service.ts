...
import type { Message, OpenAgentClient, SessionMessageResponse } from "@openagent-ai/sdk/v2"
...
export const AuthMethodID = "openagent-login"
...
export class Service extends Context.Service<Service, Interface>()("@openagent/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `openagent auth login` in the terminal",
      name: "Login with openagent",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: OpenAgentClient) {
...
function makeUsageService(sdk: OpenAgentClient) {
...
async function loadDirectorySnapshot(sdk: OpenAgentClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, openagent provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const openagentProvider = providers[ProviderV2.ID.make("openagent")]
  const openagentModel = openagentProvider ? Provider.sort(Object.values(openagentProvider.models))[0] : undefined
  if (openagentProvider && openagentModel) return { providerID: openagentProvider.id, modelID: openagentModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "OpenAgent service failure", service })
}
