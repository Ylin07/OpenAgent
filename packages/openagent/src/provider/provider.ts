...
import { Log } from "@openagent-ai/core/util/log"
import { Npm } from "@openagent-ai/core/npm"
import { Hash } from "@openagent-ai/core/util/hash"
import { Plugin } from "../plugin"
import { serviceUse } from "@openagent-ai/core/effect/service-use"
import { type LanguageModelV3 } from "@ai-sdk/provider"
import { ModelsDev } from "@openagent-ai/core/models-dev"
...
import { optionalOmitUndefined } from "@openagent-ai/core/schema"
...
const ONLY_PROVIDER = ProviderV2.ID.make("deepseek")
...
export class Service extends Context.Service<Service, Interface>()("@openagent/Provider") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        const deepseek = modelsDev.deepseek
        const catalog = mapValues(deepseek ? { deepseek } : {}, fromModelsDevProvider) as Record<ProviderV2.ID, Info>
        const database = mapValues(catalog, toPublicInfo) as Record<ProviderV2.ID, Info>
        ...
        function isProviderAllowed(providerID: ProviderV2.ID): boolean {
          if (providerID !== ONLY_PROVIDER) return false
          ...
          for (const [modelID, model] of Object.entries(provider.models ?? {})) {
            ...
            const parsedModel: Model = {
              ...
              api: {
                id: apiID,
                npm: apiNpm,
                url: model.provider?.api ?? provider?.api ?? existingModel?.api.url ?? modelsDev[id]?.api ?? "",
              },
    ...
    const listAvailable = Effect.fn("Provider.listAvailable")(() =>
      InstanceState.use(state, (s) => ({ ...s.catalog, ...s.providers })),
    )
    ...
    const getSmallModel = Effect.fn("Provider.getSmallModel")(function* (providerID: ProviderV2.ID) {
      ...
      const priority = providerID.startsWith("openagent")
        ? ["gpt-5-nano"]
        : defaultPriority
