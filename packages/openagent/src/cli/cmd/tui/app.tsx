...
import {
  ...
  OPENAGENT_BASE_MODE,
  OpenAgentKeymapProvider,
  registerOpenAgentKeymap,
  useBindings,
  useOpenAgentKeymap,
} from "./keymap"
...
export function tuiRendererConfig(_config: TuiConfig.Resolved): CliRendererConfig {
  const mouseEnabled = !Flag.OPENAGENT_DISABLE_MOUSE && (_config.mouse ?? true)
...
export function tui(input: TuiInput): TuiHandle {
  ...
  const unregisterKeymap = registerOpenAgentKeymap(keymap, renderer, input.config)
...
async function mountTui(input: TuiInput & { keymap: ReturnType<typeof createDefaultOpenTuiKeymap>; exit: Exit }) {
  ...
  await render(() => {
    return (
      ...
      >
        <OpenAgentKeymapProvider keymap={input.keymap}>
...
function App() {
  ...
  const offSelectionKeys = keymap.intercept(
    "key",
    ({ event }) => {
      if (!Flag.OPENAGENT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.OPENAGENT_DISABLE_TERMINAL_TITLE) return
  ...
  const appCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        category: "Workspace",
        hidden: !Flag.OPENAGENT_EXPERIMENTAL_WORKSPACES,
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to OpenAgent v${result.data.version}. Please restart the application.`,
    )
  ...
  return (
    <box
      ...
      onMouseUp={Flag.OPENAGENT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT ? undefined : () => Selection.copy(renderer, toast)}
    >
