...
export const OPENAGENT_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const OPENAGENT_MODE_KEY = "openagent.mode"

export const OpenAgentKeymapProvider = KeymapProvider
export const useOpenAgentKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type OpenAgentModeStack = ReturnType<typeof createOpenAgentModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, OpenAgentModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createOpenAgentModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(OPENAGENT_MODE_KEY, OPENAGENT_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(OPENAGENT_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(OPENAGENT_MODE_KEY, stack.at(-1)?.mode ?? OPENAGENT_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? OPENAGENT_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(OPENAGENT_MODE_KEY, undefined)
...
export function useOpenAgentModeStack() {
  return getOpenAgentModeStack(useOpenAgentKeymap())
}

export function getOpenAgentModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("OpenAgent mode stack is not registered for this keymap")
  return value
}
...
export function registerOpenAgentKeymap(
  keymap: OpenTuiKeymap,
  renderer: CliRenderer,
  config: Pick<TuiConfig.Resolved, "keybinds" | "leader_timeout">,
) {
  const modeStack = createOpenAgentModeStack(keymap)
