...
import type { AssistantMessage, FilePart, UserMessage } from "@openagent-ai/sdk/v2"
...
import { OPENAGENT_BASE_MODE, useBindings, useCommandShortcut, useLeaderActive, useOpenAgentKeymap } from "../../keymap"
...
function draftKey(sessionID: string | undefined) {
  ...
  const promptCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        name: "workspace.set",
        category: "Session",
        enabled: Flag.OPENAGENT_EXPERIMENTAL_WORKSPACES,
        slashName: "warp",
        run: () => {
          workspace.open()
        },
  ...
  createEffect(() => {
    ...
    input.traits = {
      ...
      ...computePromptTraits({
        ...
        captureTab: !props.compact,
      }),
    }
  })

  createEffect(() => {
    props.onModeChange?.(store.mode)
  })

  createEffect(() => {
    const target = inputTarget()
    if (!target) return
    const onFocused = () => props.onFocusChange?.(true)
    const onBlurred = () => props.onFocusChange?.(false)
    target.on("focused", onFocused)
    target.on("blurred", onBlurred)
    props.onFocusChange?.(target.focused)
    onCleanup(() => {
  ...
  return (
    <>
      <box ref={(r: BoxRenderable) => (anchor = r)} visible={props.visible !== false} width="100%">
        <box
          width="100%"
          border={props.compact ? undefined : ["top", "bottom", "left", "right"]}
          borderColor={borderHighlight()}
          backgroundColor={props.compact ? theme.background : theme.backgroundPanel}
        >
          ...
          >
            <textarea
              ...
              cursorColor={props.disabled ? theme.backgroundPanel : theme.primary}
              syntaxStyle={syntax()}
            />
          </box>
          <Show when={!props.compact}>
            ...
            <Switch>
              ...
              <Match when={workspace.notice()}>{(notice) => <text fg={theme.accent}>{notice()}</text>}</Match>
