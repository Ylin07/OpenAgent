...
export function DialogSessionList(props: { onSelect?: (sessionID: string, title: string) => void } = {}) {
  ...
  const options = createMemo(() => {
    ...
    function buildOption(id: string, category: string) {
      ...
      if (Flag.OPENAGENT_EXPERIMENTAL_WORKSPACES) {
        if (x.workspaceID) {
          footer = workspace ? (
            <WorkspaceLabel
              type={workspace.type}
  ...
  return (
    <DialogSelect
      ...
      onSelect={(option) => {
        if (props.onSelect) {
          props.onSelect(option.value, option.title)
        } else {
          route.navigate({
            type: "session",
