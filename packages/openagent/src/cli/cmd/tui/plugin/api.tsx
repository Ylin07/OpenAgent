import type { TuiDialogSelectOption, TuiPluginApi, TuiRouteDefinition, TuiSlotProps } from "@openagent-ai/plugin/tui"
...
import type { useOpenAgentKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useOpenAgentKeymap>
...
export function createTuiApi(input: Input): TuiPluginApi {
  ...
  return {
    ...
    mode: {
      current() {
        return Keymap.getOpenAgentModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getOpenAgentModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
